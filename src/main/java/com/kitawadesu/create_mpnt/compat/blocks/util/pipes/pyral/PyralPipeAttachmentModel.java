package com.kitawadesu.create_mpnt.compat.blocks.util.pipes.pyral;
...
public class PyralPipeAttachmentModel extends BakedModelWrapperWithData {

    private static final ModelProperty<PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;

    public static PyralPipeAttachmentModel withAO(BakedModel template) {
        return new PyralPipeAttachmentModel(template, true);
    }

    public static PyralPipeAttachmentModel withoutAO(BakedModel template) {
        return new PyralPipeAttachmentModel(template, false);
    }

    public PyralPipeAttachmentModel(BakedModel template, boolean ao) {
        super(template);
        this.ao = ao;
    }
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(CompatPartialModels.PYRAL_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));
        ...
        if (data.has(PIPE_PROPERTY)) {
            ...
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = CompatPartialModels.PYRAL_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(CompatPartialModels.PYRAL_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(CompatPartialModels.PYRAL_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
