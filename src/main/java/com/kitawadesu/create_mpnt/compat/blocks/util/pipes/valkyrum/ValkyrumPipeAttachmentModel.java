package com.kitawadesu.create_mpnt.compat.blocks.util.pipes.valkyrum;
...
public class ValkyrumPipeAttachmentModel extends BakedModelWrapperWithData {

    private static final ModelProperty<PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;

    public static ValkyrumPipeAttachmentModel withAO(BakedModel template) {
        return new ValkyrumPipeAttachmentModel(template, true);
    }

    public static ValkyrumPipeAttachmentModel withoutAO(BakedModel template) {
        return new ValkyrumPipeAttachmentModel(template, false);
    }

    public ValkyrumPipeAttachmentModel(BakedModel template, boolean ao) {
        super(template);
        this.ao = ao;
    }
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(CompatPartialModels.VALKYRUM_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));
        ...
        if (data.has(PIPE_PROPERTY)) {
            ...
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = CompatPartialModels.VALKYRUM_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(CompatPartialModels.VALKYRUM_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(CompatPartialModels.VALKYRUM_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
