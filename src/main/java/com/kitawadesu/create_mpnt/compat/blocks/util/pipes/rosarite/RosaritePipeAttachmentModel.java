package com.kitawadesu.create_mpnt.compat.blocks.util.pipes.rosarite;
...
public class RosaritePipeAttachmentModel extends BakedModelWrapperWithData {

    private static final ModelProperty<PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;

    public static RosaritePipeAttachmentModel withAO(BakedModel template) {
        return new RosaritePipeAttachmentModel(template, true);
    }

    public static RosaritePipeAttachmentModel withoutAO(BakedModel template) {
        return new RosaritePipeAttachmentModel(template, false);
    }

    public RosaritePipeAttachmentModel(BakedModel template, boolean ao) {
        super(template);
        this.ao = ao;
    }
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(CompatPartialModels.ROSARITE_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));
        ...
        if (data.has(PIPE_PROPERTY)) {
            ...
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = CompatPartialModels.ROSARITE_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(CompatPartialModels.ROSARITE_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(CompatPartialModels.ROSARITE_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
