package com.kitawadesu.create_mpnt.compat.blocks.util.pipes.rose_gold;
...
public class RoseGoldPipeAttachmentModel extends BakedModelWrapperWithData {

    private static final ModelProperty<PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;

    public static RoseGoldPipeAttachmentModel withAO(BakedModel template) {
        return new RoseGoldPipeAttachmentModel(template, true);
    }

    public static RoseGoldPipeAttachmentModel withoutAO(BakedModel template) {
        return new RoseGoldPipeAttachmentModel(template, false);
    }

    public RoseGoldPipeAttachmentModel(BakedModel template, boolean ao) {
        super(template);
        this.ao = ao;
    }
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(CompatPartialModels.ROSE_GOLD_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));
        ...
        if (data.has(PIPE_PROPERTY)) {
            ...
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = CompatPartialModels.ROSE_GOLD_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(CompatPartialModels.ROSE_GOLD_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(CompatPartialModels.ROSE_GOLD_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
