...
public class IronPipeAttachmentModel extends BakedModelWrapperWithData {
    ...
    private static final ModelProperty<IronPipeAttachmentModel.PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;
    ...
    protected ModelData.Builder gatherModelData(ModelData.Builder builder, BlockAndTintGetter world, BlockPos pos, BlockState state,
        ...
        IronPipeAttachmentModel.PipeModelData data = new IronPipeAttachmentModel.PipeModelData();
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(MPNTPartialModels.IRON_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));

        if (data.has(PIPE_PROPERTY)) {
            IronPipeAttachmentModel.PipeModelData pipeData = data.get(PIPE_PROPERTY);
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = MPNTPartialModels.IRON_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
                          IronPipeAttachmentModel.PipeModelData pipeData, RenderType renderType) {
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(MPNTPartialModels.IRON_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(MPNTPartialModels.IRON_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
