...
public class AndesitePipeAttachmentModel extends BakedModelWrapperWithData {

    private static final ModelProperty<AndesitePipeAttachmentModel.PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;

    public static AndesitePipeAttachmentModel withAO(BakedModel template) {
        return new AndesitePipeAttachmentModel(template, true);
    }

    public static AndesitePipeAttachmentModel withoutAO(BakedModel template) {
        return new AndesitePipeAttachmentModel(template, false);
    }

    public AndesitePipeAttachmentModel(BakedModel template, boolean ao) {
        super(template);
        this.ao = ao;
    }
    ...
    protected ModelData.Builder gatherModelData(ModelData.Builder builder, BlockAndTintGetter world, BlockPos pos, BlockState state,
        ...
        AndesitePipeAttachmentModel.PipeModelData data = new AndesitePipeAttachmentModel.PipeModelData();
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(MPNTPartialModels.ANDESITE_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));

        if (data.has(PIPE_PROPERTY)) {
            AndesitePipeAttachmentModel.PipeModelData pipeData = data.get(PIPE_PROPERTY);
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = MPNTPartialModels.ANDESITE_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
                          AndesitePipeAttachmentModel.PipeModelData pipeData, RenderType renderType) {
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(MPNTPartialModels.ANDESITE_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(MPNTPartialModels.ANDESITE_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
