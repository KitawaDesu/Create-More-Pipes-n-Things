package com.kitawadesu.create_mpnt.blocks.util.pipes.gold;
...
public class GoldPipeAttachmentModel extends BakedModelWrapperWithData {

    private static final ModelProperty<GoldPipeAttachmentModel.PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;

    public static GoldPipeAttachmentModel withAO(BakedModel template) {
        return new GoldPipeAttachmentModel(template, true);
    }

    public static GoldPipeAttachmentModel withoutAO(BakedModel template) {
        return new GoldPipeAttachmentModel(template, false);
    }

    public GoldPipeAttachmentModel(BakedModel template, boolean ao) {
        super(template);
        this.ao = ao;
    }
    ...
    protected ModelData.Builder gatherModelData(ModelData.Builder builder, BlockAndTintGetter world, BlockPos pos, BlockState state,
        ...
        GoldPipeAttachmentModel.PipeModelData data = new GoldPipeAttachmentModel.PipeModelData();
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(MPNTPartialModels.GOLD_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));

        if (data.has(PIPE_PROPERTY)) {
            GoldPipeAttachmentModel.PipeModelData pipeData = data.get(PIPE_PROPERTY);
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = MPNTPartialModels.GOLD_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
                          GoldPipeAttachmentModel.PipeModelData pipeData, RenderType renderType) {
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(MPNTPartialModels.GOLD_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(MPNTPartialModels.GOLD_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
