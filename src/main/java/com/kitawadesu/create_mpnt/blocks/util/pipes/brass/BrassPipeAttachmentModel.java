package com.kitawadesu.create_mpnt.blocks.util.pipes.brass;
...
public class BrassPipeAttachmentModel extends BakedModelWrapperWithData {
    ...
    private static final ModelProperty<BrassPipeAttachmentModel.PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
    private boolean ao;

    public static BrassPipeAttachmentModel withAO(BakedModel template) {
        return new BrassPipeAttachmentModel(template, true);
    }

    public static BrassPipeAttachmentModel withoutAO(BakedModel template) {
        return new BrassPipeAttachmentModel(template, false);
    }

    public BrassPipeAttachmentModel(BakedModel template, boolean ao) {
        super(template);
        this.ao = ao;
    }
    ...
    protected ModelData.Builder gatherModelData(ModelData.Builder builder, BlockAndTintGetter world, BlockPos pos, BlockState state,
        ...
        BrassPipeAttachmentModel.PipeModelData data = new BrassPipeAttachmentModel.PipeModelData();
    ...
    public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
        ...
        set.add(MPNTPartialModels.BRASS_FLUID_PIPE_CASING.get().getRenderTypes(state, rand, data));

        if (data.has(PIPE_PROPERTY)) {
            BrassPipeAttachmentModel.PipeModelData pipeData = data.get(PIPE_PROPERTY);
            for (Direction d : Iterate.directions) {
                ...
                for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                    ChunkRenderTypeSet attachmentRenderTypeSet = MPNTPartialModels.BRASS_PIPE_ATTACHMENTS.get(partial).get(d)
    ...
    private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
                          BrassPipeAttachmentModel.PipeModelData pipeData, RenderType renderType) {
        ...
        for (Direction d : Iterate.directions) {
            FluidTransportBehaviour.AttachmentTypes type = pipeData.getAttachment(d);
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
                quads.addAll(MPNTPartialModels.BRASS_PIPE_ATTACHMENTS.get(partial)
                        .get(d)
                        .get()
        ...
        if (pipeData.isEncased())
            quads.addAll(MPNTPartialModels.BRASS_FLUID_PIPE_CASING.get()
                    .getQuads(state, side, rand, data, renderType));
    }
